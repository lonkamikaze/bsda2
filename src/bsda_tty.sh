...
#
# Harvest the terminal control daemon.
#
bsda:tty:Async.clean() {
	local dpid
	# Harvest the background daemon
	$this.getDpid dpid
	if [ -n "$dpid" ]; then
		kill $dpid 2>&-
		wait $dpid || :
...
#
# The stdout() and # stderr() methods still produce output.
#
bsda:tty:Async.deactivate() {
	local fifo
	$this.Fifo fifo
	$fifo.sink echo deactivate
}
...
bsda:tty:Async:daemon() {
	local fifo term cmd arg retval
	bsda:obj:fork
	$this.Fifo fifo
	...
	while true; do
		...
		if [ $retval -gt 128 ]; then
			...
			$term.stderr "bsda:tty:Async.daemon: ERROR: Read from pipe returned: $retval"
			exit $retval
		fi
		arg="$(printf "$arg")" # Deserialise
		case "$cmd" in
		line[0-9]*)
			$term.${cmd%%[0-9]*} ${cmd#${cmd%%[0-9]*}} "$arg"
		;;
		stdout | stderr | use | deactivate)
			$term.$cmd "$arg"
		;;
		*)
			$term.stderr "bsda:tty:Async.daemon: WARNING: Illegal command received: $(echo -n "$cmd" | bsda:obj:escape)"
		;;
		esac
	done
}
